...
async def do_pm_options_action(event, chat):
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = f"- اتذكر حذرتك وقلت لك لا تكرر اكثر من خيار ولا ترسل رسائل مره اخرى فقط انتظر. \
اسف سوف احظرك من الحساب. \
حاليا لا تستطيع بعد تتكلم الى ان ياتي صاحب الحساب ويقوم بالغاء الحظر. 🙂💘"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """ههاه لازم تصبر مالك الحساب مشافش الرسالة انتظر. \
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5768182096:
        await event.client.send_message(chat, "- اهلا بك عزيزي المبرمج صلاح مبرمج سورس تلي جود 🖤 ")
