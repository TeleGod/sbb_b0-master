    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/36f4ee18f981a2a86adba.jpg",
                caption="**تم تشغيل سورس تلي جود بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("جروب المساعدة", "https://t.me/te_god"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            ...
            LOGS.error("لم يتم التعرف على فار جروب الحفظ")
        except TypeError:
            LOGS.error("يبدو انك وضعت فار جروب الحفظ بشكل غير صحيح")
        except Exception as e:
            LOGS.error("هنالك خطا ما للتعرف على فار جروب الحفظ\n" + str(e))
    else:
        descript = "⪼ هذه هي مجموعه الحفظ الخاصه بك لا تحذفها ابدا  𓆰."
        photobt = await sbb_b.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "جروب بوت تلي جود", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            ...
            LOGS.error(
                "لم يتم العثور على ايدي جروب التخزين تاكد من انه مكتوب بشكل صحيح "
            )
        except TypeError:
            LOGS.error("صيغه ايدي جروب التخزين غير صالحة.تاكد من انه مكتوب بشكل صحيح ")
        except Exception as e:
            LOGS.error("حدث خطأ اثناء التعرف على جروب التخزين\n" + str(e))
    else:
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if JMTHONBRANCH := branch:
        ...
        errtext = f"لا يوجد فرع بأسم `{JMTHONBRANCH}` في الريبو الخارجي {JMTHONREPO}. تاكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = JMTHONREPO
